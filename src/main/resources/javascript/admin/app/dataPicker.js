angular.module('dataPicker', [])
        .controller('dataPickerCtrl', ['$scope', function ($scope) {
                $scope.tree = [];
                $scope.selectedPath = "";
                $scope.propName = "";
                $scope.selectMessage = "";
                ...
                $scope.transformFlatListToNodeTree = function (flatList) {
                    ...
                    if (flatList) {
                        for (var i = 0; i < flatList.length; i++) {
                            var chain = flatList[i].path.substring(1).split("/");
